...
def flatten(lst):
    ...
    for element in lst:
        if type(element) == list:
            flattened_elem = flatten(element)
            flattened_lst.extend(flattened_elem)
        else:
...
def merge(lst1, lst2):
    ...
    [2, 4, 5, 6, 7]
    """
    #result = flatten([lst1, lst2])
    #result.sort()
    #return result
    l1, l2 = len(lst1), len(lst2) # get length of each list
    i, j, result = 0, 0, [] # i and j are index of lst1 and lst2, result is the merged list
    while i < l1 or j < l2: # stop when BOTH indices are out of range of lists
        if i < l1 and (l2 <= j or lst1[i] <= lst2[j]):
            result, i = result + [lst1[i]], i + 1
        if j < l2 and (l1 <= i or lst2[j] < lst1[i]):
            result, j = result + [lst2[j]], j + 1
    return result
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    for row in range(max_rows-1, -1, -1): # because the uppermost row is labled 0
        piece = get_piece(board, row, column)
        if piece == '-':
...
def print_board(board, max_rows, max_cols):
    ...
    for i in range(max_rows):
        ...
        for j in range(max_cols):
            pieces = pieces + ' ' + get_piece(board, i, j)
        print(pieces.strip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for i in range(max_cols-num_connect+1):
        ...
        if piece == player:
            cnt = 1
            lst = [player for j in range(i+1, i+num_connect) if get_piece(board, row, j) == player]
            if cnt + len(lst) == num_connect:
                return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for i in range(max_rows-num_connect+1):
        ...
        if piece == player:
            cnt = 1
            lst = [player for j in range(i+1, i+num_connect) if get_piece(board, j, col) == player]
            if cnt + len(lst) == num_connect:
                return True
    return False
