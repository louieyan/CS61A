...
def roll_dice(num_rolls, dice=six_sided):
    ...
    pig_out = False
    while k <= num_rolls:
        num_to_be_added = dice()
        if num_to_be_added == 1:
            pig_out = True
        k, total = k + 1, total + num_to_be_added
    if pig_out:
        return 1
    else:
        return total 
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while not gameover:
        ...
        gameover = score0 >= goal or score1 >= goal
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(scores0, scores1):
        # important
        previous_high_s, previous_score_s = previous_high, previous_score
        if who == 1:
            changes = scores1 - previous_score_s
            previous_score_s = scores1
        else:
            changes = scores0 - previous_score_s
            previous_score_s = scores0
        if previous_high_s < changes:
            if changes == 1:
                print(f"1 point! That's the biggest gain yet for Player {who}")
            else:
                print(f"{changes} points! That's the biggest gain yet for Player {who}")
            previous_high_s = changes
        return announce_highest(who, previous_high_s, previous_score_s)
    return say
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    pre_max, max_rolls = 0, 0
    for num_rolls in range(1, 11):
        averaged_dice = make_averaged(roll_dice, num_samples)
        average_value = averaged_dice(num_rolls, dice)
        if average_value > pre_max:
            pre_max = average_value
            max_rolls = num_rolls
    return max_rolls
    # END PROBLEM 9
