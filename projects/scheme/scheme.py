...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if scheme_length(formals) != scheme_length(vals):
            raise SchemeError('the numbers of parameters and values are not equal')
...
def do_and_form(expressions, env):
    ...
    if scheme_truep(val):
        if expressions.second is nil: # if it's the last expr
            return val # return it value
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal, val = binding.first, scheme_eval(binding.second.first, env)
